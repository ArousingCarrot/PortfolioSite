...
export default async function AdminPage() {
  ...
  return (
    <div>
      ...
      {dbError && (
        <p className="text-sm text-red-400 mb-6">Failed to load analytics data.</p>
      )}
            ...
            <tbody className="divide-y divide-neutral-900">
              {recentEvents.map((e, i) => (
                <tr key={i} className="text-neutral-300">
                  ...
                  <td className="py-3 pr-6">{e.country ?? "-"}</td>
                  <td className="py-3 text-neutral-500 text-xs">{e.ts}</td>
                </tr>
              ))}
            </tbody>
          </table>
